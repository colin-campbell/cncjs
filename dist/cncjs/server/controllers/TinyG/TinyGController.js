...
var _ensureType2 = require("../../lib/ensure-type");
...
var TinyGController = /*#__PURE__*/function () {
  ...
  function TinyGController(engine, options) {
    ...
    this.feeder = new _Feeder["default"]({
      ...
      dataFilter: function dataFilter(line, context) {
         ...
         */
        var parts = originalLine.split(/;([\s\S]*)/); // `s` is the modifier for single-line mode
        line = (0, _ensureType.ensureString)(parts[0]).trim();
        ...
        var words = (0, _ensureType.ensureArray)(data.words);
        {
          // Program Mode: M0, M1
      ...
      dataFilter: function dataFilter(line, context) {
        ...
        var words = (0, _ensureType.ensureArray)(data.words);
        {
          ...
          if (programMode === 'M0') {
            ...
            _this.event.trigger('gcode:pause');
    ...
    this.runner.on('r', function (r) {
      ...
      console.assert(_this.workflow.state !== _Workflow.WORKFLOW_STATE_RUNNING, "workflow.state !== '".concat(_Workflow.WORKFLOW_STATE_RUNNING, "'"));
    ...
    this.timer.query = setInterval(function () {
      ...
      var zeroOffset = (0, _isEqual2["default"])(_this.runner.getWorkPosition(_this.state), _this.runner.getWorkPosition(_this.runner.state));
  ...
  _createClass(TinyGController, [{
    key: "initController",
    value: function () {
      var _initController = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
        var _this2 = this;
    ...
    value: function populateContext(context) {
      ...
      var _this$runner$getWorkP = this.runner.getWorkPosition(),
        ...
        posc = _this$runner$getWorkP.c;

      // Modal group
      var modal = this.runner.getModalGroup();
      ...
      return Object.assign(context || {}, _objectSpread({
        ...
        modal: {
          ...
          coolant: (0, _ensureType.ensureArray)(modal.coolant).join('\n')
        },
        // Tool
        tool: Number(tool) || 0
    ...
    value: function open() {
      ...
      var _this$options = this.options,
        ...
        baudrate = _this$options.baudrate;
    ...
    value: function command(cmd) {
      ...
      var handler = {
        ...
        'gcode:stop': function gcodeStop() {
          ...
          if (force) {
            var firmwareBuild = (0, _ensureType2.ensureNumber)((0, _get2["default"])(_this5.settings, 'fb'));
        ...
        'gcode': function gcode() {
          var commands = args[0],
            context = args[1];
          var data = (0, _ensureType.ensureArray)(commands).join('\n').split(/\r?\n/).filter(function (line) {
