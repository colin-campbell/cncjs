...
var createServer = function createServer(options, callback) {
  ...
  var rcfile = _path["default"].resolve(options.configFile || _settings["default"].rcfile);

  // configstore service
  log.info("Loading configuration from ".concat(_chalk["default"].yellow(JSON.stringify(rcfile))));
  _configstore["default"].load(rcfile);
  ...
  }
  {
    ...
    if (watchDirectory) {
      if (_fs["default"].existsSync(watchDirectory)) {
        log.info("Watching ".concat(_chalk["default"].yellow(JSON.stringify(watchDirectory)), " for file changes"));
